...
import org.oss.pdfreporter.crosstabs.fill.JRCrosstabExpressionEvaluator;
import org.oss.pdfreporter.crosstabs.fill.JRFillCrosstabCell;
import org.oss.pdfreporter.crosstabs.fill.JRFillCrosstabColumnGroup;
import org.oss.pdfreporter.crosstabs.fill.JRFillCrosstabGroup;
import org.oss.pdfreporter.crosstabs.fill.JRFillCrosstabMeasure;
import org.oss.pdfreporter.crosstabs.fill.JRFillCrosstabObjectFactory;
...
/**
 * Fill-time implementation of a {@link org.oss.pdfreporter.crosstabs.JRCrosstab crosstab}.
 *
 * @author Lucian Chirita (lucianc@users.sourceforge.net)
 * @version $Id: JRFillCrosstab.java 5661 2012-09-13 19:00:14Z teodord $
 */
public class JRFillCrosstab extends JRFillElement implements JRCrosstab, JROriginProvider, BucketingServiceContext
{
	private final static Logger logger = Logger.getLogger(JRFillCrosstab.class.getName());
	...
	protected Array2D<? extends JRFillCrosstabCell> crossCells;
	...
	private void copyParameters(JRCrosstab crosstab, JRFillObjectFactory factory)
	{
		...
		parametersMap = new HashMap<String,IJRFillParameter>();
	...
	{
		Array2D<? extends JRCrosstabCell> crosstabCells = crosstab.getCells();
		crossCells = new Array2DImpl<JRFillCrosstabCell>(rowGroups.length + 1,columnGroups.length + 1);
		for (int i = 0; i <= rowGroups.length; ++i)
		{
			for (int j = 0; j <= columnGroups.length; ++j)
			{
				if (crosstabCells.get(i, j) != null)
				{
					crossCells.set(i, j, factory.getCrosstabCell(crosstabCells.get(i, j)));
				}
			}
		}
	}
	...
	protected void loadEvaluator(JasperReport jasperReport)
	{
		try
		{
			JREvaluator evaluator = (JREvaluator) JasperCompileManager.getInstance(filler.getJasperReportsContext()).getEvaluator(jasperReport, parentCrosstab);
	...
	protected class CrosstabFiller
	{
		...
		private boolean prepareDataCell(CrosstabCell data, int column, int availableHeight, int xOffset) throws JRException
		{
			...
			JRFillCrosstabCell cell =  crossCells.get(data.getRowTotalGroupIndex(), data.getColumnTotalGroupIndex());
