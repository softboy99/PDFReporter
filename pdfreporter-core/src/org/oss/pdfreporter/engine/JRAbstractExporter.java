...
import org.oss.pdfreporter.engine.export.ExporterFilterFactory;
 ...
 * @version $Id: JRAbstractExporter.java 5526 2012-08-01 11:36:41Z teodord $
 */
public abstract class JRAbstractExporter implements JRExporter
{
	 ...
	 *
	 */
	protected Map<String, IDateFormat> dateFormatCache = new HashMap<String, IDateFormat>();
	protected Map<String, INumberFormat> numberFormatCache = new HashMap<String, INumberFormat>();
	...
	protected void setInput() throws JRException
	{
		...
		if (jasperPrintList == null)
		{
			...
			if (jasperPrint == null)
			{
				...
				{
					IURL url = (IURL)parameters.get(JRExporterParameter.INPUT_URL);
					if (url != null)
					{
...
//		return textValue;
	...
	protected IDateFormat getDateFormat(String formatFactoryClass, String pattern, StringLocale lc, TimeZone tz)
	{
		...
		IDateFormat dateFormat = dateFormatCache.get(key);
		if (dateFormat == null)
		{
	...
	protected INumberFormat getNumberFormat(String formatFactoryClass, String pattern, StringLocale lc)
	{
		String key = formatFactoryClass 
			+ "|" + pattern 
			+ "|" + (lc == null ? "" : JRDataUtils.getLocaleCode(lc)); 
		INumberFormat numberFormat = numberFormatCache.get(key);
