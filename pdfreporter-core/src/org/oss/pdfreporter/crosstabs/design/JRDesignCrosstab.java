...
/**
 * Design-time {@link org.oss.pdfreporter.crosstabs.JRCrosstab crosstab} implementation.
 *
 * @author Lucian Chirita (lucianc@users.sourceforge.net)
 * @version $Id: JRDesignCrosstab.java 5180 2012-03-29 13:23:12Z teodord $
 */
public class JRDesignCrosstab extends JRDesignElement implements JRCrosstab
{
	private final static Logger logger = Logger.getLogger(JRDesignCrosstab.class.getName());
	...
	protected Array2D<JRDesignCrosstabCell> crossCells;
	...
	*/
	...
	private static final Object[] BUILT_IN_PARAMETERS = new Object[] {
		...
		// TODO: Daniel (19.4.2013) - Removed, deprecated and contains classes that can't be translated
	...
	protected void createCellMatrix()
	{
		...
		for (Iterator<JRCrosstabCell> it = cellsList.iterator(); it.hasNext();)
		{
			...
			crossCells.set(rowGroupIndex, columnGroupIndex, crosstabCell);
		}
	...
	protected void inheritCells()
	{
		for (int i = rowGroups.size(); i >= 0 ; --i)
		{
			for (int j = columnGroups.size(); j >= 0 ; --j)
			{
				...
				if (used)
				{
					if (crossCells.get(i, j) == null)
					{
						inheritCell(i, j);

						if (crossCells.get(i, j) == null)
						{
							crossCells.set(i, j, emptyCell(i, j));
							inheritCellSize(i, j);
						}
					}
					else
	...
	{
		JRDesignCrosstabCell cell = crossCells.get(i, j);
		...
		if (contents.getWidth() == JRCellContents.NOT_CALCULATED)
		{
			if (i < rowGroups.size())
			{
				JRDesignCrosstabCell rowCell = crossCells.get(rowGroups.size(), j);
				if (rowCell != null)
				{
			...
			else
			{
				...
				{
					if (crossCells.get(i, k) != null)
					{
						contents.setWidth(crossCells.get(i, k).getContents().getWidth());
						break;
					}
				}
			}
		...
		if (contents.getHeight() == JRCellContents.NOT_CALCULATED)
		{
			if (j < columnGroups.size())
			{
				JRDesignCrosstabCell colCell = crossCells.get(i, columnGroups.size());
				if (colCell != null)
				{
			...
			else
			{
				...
				{
					if (crossCells.get(k, j) != null)
					{
						contents.setHeight(crossCells.get(k, j).getContents().getHeight());
					}
				}
			}
		}
	...
	protected void inheritCell(int i, int j)
	{
		...
		if (j < columnGroups.size())
		{
			JRDesignCrosstabCell colCell = crossCells.get(rowGroups.size(), j);
			...
			{
				JRDesignCrosstabCell cell = crossCells.get(i, k);
				if (cell != null)
				{
					JRDesignCellContents contents = (JRDesignCellContents) cell.getContents();
		...
		{
			JRDesignCrosstabCell rowCell = crossCells.get(i, columnGroups.size());
			...
			{
				JRDesignCrosstabCell cell = crossCells.get(k, j);
				if (cell != null)
				{
					JRDesignCellContents contents = (JRDesignCellContents) cell.getContents();
		...
		crossCells.set(i, j, inheritedCell);
	}
	...
	protected int calculateRowHeadersSizes()
	{
		...
		for (int i = rowGroups.size() - 1, heightSum = 0; i >= 0; --i)
		{
			...
			JRDesignCrosstabCell cell = crossCells.get(i + 1,columnGroups.size());
			if (cell != null)
			{
			...
			if (group.hasTotal())
			{
				...
				JRDesignCrosstabCell totalCell = crossCells.get(i, columnGroups.size());
				if (totalCell != null)
				{
	...
	protected int calculateColumnHeadersSizes()
	{
		...
		for (int i = columnGroups.size() - 1, widthSum = 0; i >= 0; --i)
		{
			...
			JRDesignCrosstabCell cell = crossCells.get(rowGroups.size(),i + 1);
			if (cell != null)
			{
			...
			if (group.hasTotal())
			{
				...
				JRDesignCrosstabCell totalCell = crossCells.get(rowGroups.size(),i);
				if (totalCell != null)
				{
